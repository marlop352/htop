/*
htop - CygwinProcessList.c
(C) 2014 Hisham H. Muhammad
Released under the GNU GPL, see the COPYING file
in the source distribution for its full text.
*/

#include "CygwinProcessList.h"
...
typedef struct CygwinProcessList_ {
   ProcessList super;
...
static void CygwinProcessList_initTtyDrivers(CygwinProcessList* this) {
   TtyDriver* ttyDrivers;
...
static void CygwinProcessList_initNetlinkSocket(CygwinProcessList* this) {
...
ProcessList* ProcessList_new(UsersTable* usersTable, Hashtable* pidWhiteList, uid_t userId) {
   CygwinProcessList* this = xCalloc(1, sizeof(CygwinProcessList));
   ProcessList* pl = &(this->super);

   ProcessList_init(pl, Class(CygwinProcess), usersTable, pidWhiteList, userId);
   CygwinProcessList_initTtyDrivers(this);

   #ifdef HAVE_DELAYACCT
   CygwinProcessList_initNetlinkSocket(this);
   #endif
...
void ProcessList_delete(ProcessList* pl) {
   CygwinProcessList* this = (CygwinProcessList*) pl;
   ProcessList_done(pl);
   free(this->cpus);
   if (this->ttyDrivers) {
...
static inline unsigned long long CygwinProcess_adjustTime(unsigned long long t) {
   if(jiffy == 0.0) jiffy = sysconf(_SC_CLK_TCK);
   double jiffytime = 1.0 / jiffy;
   return (unsigned long long) t * jiffytime * 100;
}

static bool CygwinProcessList_readStatFile(Process *process, const char* dirname, const char* name, char* command, int* commLen) {
   CygwinProcess* lp = (CygwinProcess*) process;
   ...
   *commLen = commsize;
   ...
   location += 1;
   lp->utime = CygwinProcess_adjustTime(strtoull(location, &location, 10));
   location += 1;
   lp->stime = CygwinProcess_adjustTime(strtoull(location, &location, 10));
   location += 1;
   lp->cutime = CygwinProcess_adjustTime(strtoull(location, &location, 10));
   location += 1;
   lp->cstime = CygwinProcess_adjustTime(strtoull(location, &location, 10));
   location += 1;
...
static bool CygwinProcessList_statProcessDir(Process* process, const char* dirname, char* name) {
   char filename[MAX_NAME+1];
   filename[MAX_NAME] = '\0';
...
static void CygwinProcessList_readIoFile(CygwinProcess* process, const char* dirname, char* name, unsigned long long now) {
   char filename[MAX_NAME+1];
   filename[MAX_NAME] = '\0';
...
static bool CygwinProcessList_readStatmFile(CygwinProcess* process, const char* dirname, const char* name) {
   char filename[MAX_NAME+1];
...
static void CygwinProcessList_readOpenVZData(CygwinProcess* process, const char* dirname, const char* name) {
...
static void CygwinProcessList_readCGroupFile(CygwinProcess* process, const char* dirname, const char* name) {
   char filename[MAX_NAME+1];
...
static void CygwinProcessList_readVServerData(CygwinProcess* process, const char* dirname, const char* name) {
   char filename[MAX_NAME+1];
...
static void CygwinProcessList_readOomData(CygwinProcess* process, const char* dirname, const char* name) {
   char filename[MAX_NAME+1];
...
static int handleNetlinkMsg(struct nl_msg *nlmsg, void *CygwinProcess) {
   ...
   CygwinProcess* lp = (CygwinProcess*) CinuxProcess;
...
static void CygwinProcessList_readDelayAcctData(CygwinProcessList* this, CygwinProcess* process) {
   struct nl_msg *msg;
...
static bool CygwinProcessList_readCmdlineFile(Process* process, const char* dirname, const char* name) {
   char filename[MAX_NAME+1];
   ...
   int lastChar = 0;
   if (amtRead == 0) {
      ((CygwinProcess*)process)->isKernelThread = true;
      return true;
   } else if (amtRead < 0) {
      return false;
   }
...
static char* CygwinProcessList_updateTtyDevice(TtyDriver* ttyDrivers, unsigned int tty_nr) {
...
static bool CygwinProcessList_recurseProcTree(CygwinProcessList* this, const char* dirname, Process* parent, double period, struct timeval tv) {
   ...
   while ((entry = readdir(dir)) != NULL) {
      ...
      Process* proc = ProcessList_getProcess(pl, pid, &preExisting, (Process_New) CygwinProcess_new);
      proc->tgid = parent ? parent->pid : pid;
      
      CygwinProcess* lp = (CygwinProcess*) proc;

      char subdirname[MAX_NAME+1];
      xSnprintf(subdirname, MAX_NAME, "%s/%s/task", dirname, name);
      CygwinProcessList_recurseProcTree(this, subdirname, proc, period, tv);

      #ifdef HAVE_TASKSTATS
      if (settings->flags & PROCESS_FLAG_IO)
         CygwinProcessList_readIoFile(lp, dirname, name, now);
      #endif

      if (! CygwinProcessList_readStatmFile(lp, dirname, name))
      ...
      if (! CygwinProcessList_readStatFile(proc, dirname, name, command, &commLen))
         goto errorReadingProcess;
      if (tty_nr != proc->tty_nr && this->ttyDrivers) {
         free(lp->ttyDevice);
         lp->ttyDevice = CygwinProcessList_updateTtyDevice(this->ttyDrivers, proc->tty_nr);
      }
      if (settings->flags & PROCESS_FLAG_CYGWIN_IOPRIO)
         CygwinProcess_updateIOPriority(lp);
      ...
      if(!preExisting) {
         ...
         if (! CygwinProcessList_statProcessDir(proc, dirname, name))
            goto errorReadingProcess;

         proc->user = UsersTable_getRef(pl->usersTable, proc->st_uid);

         #ifdef HAVE_OPENVZ
         if (settings->flags & PROCESS_FLAG_CYGWIN_OPENVZ) {
            CygwinProcessList_readOpenVZData(lp, dirname, name);
         }
         #endif
         
         #ifdef HAVE_VSERVER
         if (settings->flags & PROCESS_FLAG_CYGWIN_VSERVER) {
            CygwinProcessList_readVServerData(lp, dirname, name);
         }
         #endif

         if (! CygwinProcessList_readCmdlineFile(proc, dirname, name)) {
            goto errorReadingProcess;
         }
      ...
      #ifdef HAVE_DELAYACCT
      CygwinProcessList_readDelayAcctData(this, lp);
      #endif

      #ifdef HAVE_CGROUP
      if (settings->flags & PROCESS_FLAG_CYGWIN_CGROUP)
         CygwinProcessList_readCGroupFile(lp, dirname, name);
      #endif
      
      if (settings->flags & PROCESS_FLAG_CYGWIN_OOM)
         CygwinProcessList_readOomData(lp, dirname, name);
...
static inline void CygwinProcessList_scanMemoryInfo(ProcessList* this) {
...
static inline double CygwinProcessList_scanCPUTime(CygwinProcessList* this) {
...
void ProcessList_goThroughEntries(ProcessList* super) {
   CygwinProcessList* this = (CygwinProcessList*) super;

   CygwinProcessList_scanMemoryInfo(super);
   double period = CygwinProcessList_scanCPUTime(this);

   struct timeval tv;
   gettimeofday(&tv, NULL);
   CygwinProcessList_recurseProcTree(this, PROCDIR, NULL, period, tv);
}
