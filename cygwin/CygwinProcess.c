...
#define PROCESS_FLAG_CYGWIN_IOPRIO   0x0100
#define PROCESS_FLAG_CYGWIN_OPENVZ   0x0200
#define PROCESS_FLAG_CYGWIN_VSERVER  0x0400
#define PROCESS_FLAG_CYGWIN_CGROUP   0x0800
#define PROCESS_FLAG_CYGWIN_OOM      0x1000
...
typedef enum CygwinProcessFields {
   CMINFLT = 11,
   CMAJFLT = 13,
   UTIME = 14,
   STIME = 15,
...
#define Process_isKernelThread(_process) (((CygwinProcess*)(_process))->isKernelThread)
#endif
...
#ifdef HAVE_OPENVZ
   [CTID] = { .name = "CTID", .title = "   CTID ", .description = "OpenVZ container ID (a.k.a. virtual environment ID)", .flags = PROCESS_FLAG_CYGWIN_OPENVZ, },
   [VPID] = { .name = "VPID", .title = " VPID ", .description = "OpenVZ process ID", .flags = PROCESS_FLAG_CYGWIN_OPENVZ, },
#endif
#ifdef HAVE_VSERVER
   [VXID] = { .name = "VXID", .title = " VXID ", .description = "VServer process ID", .flags = PROCESS_FLAG_CYGWIN_VSERVER, },
#endif
#ifdef HAVE_TASKSTATS
...
#endif
#ifdef HAVE_CGROUP
   [CGROUP] = { .name = "CGROUP", .title = "    CGROUP ", .description = "Which cgroup the process is in", .flags = PROCESS_FLAG_CYGWIN_CGROUP, },
#endif
   [OOM] = { .name = "OOM", .title = "    OOM ", .description = "OOM (Out-of-Memory) killer score", .flags = PROCESS_FLAG_CYGWIN_OOM, },
   [IO_PRIORITY] = { .name = "IO_PRIORITY", .title = "IO ", .description = "I/O priority", .flags = PROCESS_FLAG_CYGWIN_IOPRIO, },
#ifdef HAVE_DELAYACCT
...
ProcessClass CygwinProcess_class = {
   .super = {
      .extends = Class(Process),
      .display = Process_display,
      .delete = Process_delete,
      .compare = CygwinProcess_compare
   },
   .writeField = (Process_WriteField) CygwinProcess_writeField,
};

CygwinProcess* CygwinProcess_new(Settings* settings) {
...
*/
#define CygwinProcess_effectiveIOPriority(p_) (IOPriority_class(p_->ioPriority) == IOPRIO_CLASS_NONE ? IOPriority_tuple(IOPRIO_CLASS_BE, (p_->super.nice + 20) / 5) : p_->ioPriority)

IOPriority CygwinProcess_updateIOPriority(CygwinProcess* this) {
   IOPriority ioprio = 0;
...
bool CygwinProcess_setIOPriority(CygwinProcess* this, IOPriority ioprio) {
// Other OSes masquerading as Linux (NetBSD?) don't have this syscall
#ifdef SYS_ioprio_set
   syscall(SYS_ioprio_set, IOPRIO_WHO_PROCESS, this->super.pid, ioprio);
#endif
   return (CygwinProcess_updateIOPriority(this) == ioprio);
}

#ifdef HAVE_DELAYACCT
void CygwinProcess_printDelay(float delay_percent, char* buffer, int n) {
...
void CygwinProcess_writeField(Process* this, RichString* str, ProcessField field) {
   CygwinProcess* lp = (CygwinProcess*) this;
   ...
   #ifdef HAVE_DELAYACCT
   case PERCENT_CPU_DELAY: CygwinProcess_printDelay(lp->cpu_delay_percent, buffer, n); break;
   case PERCENT_IO_DELAY: CygwinProcess_printDelay(lp->blkio_delay_percent, buffer, n); break;
   case PERCENT_SWAP_DELAY: CygwinProcess_printDelay(lp->swapin_delay_percent, buffer, n); break;
   #endif
   default:
...
long CygwinProcess_compare(const void* v1, const void* v2) {
   CygwinProcess *p1, *p2;
   ...
   #endif
   case IO_PRIORITY:
      return CygwinProcess_effectiveIOPriority(p1) - CygwinProcess_effectiveIOPriority(p2);
   default:
